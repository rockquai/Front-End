 ...
 * ----------------------------------------------------------------------------
 * isType()                  : Javascript의 모든 데이터 유형을 올바르게 감지
 * equal()                   : 데이터 간 동등한지 유무 파악 함수
 * strictEqual()             : 데이터 간 완전하게 동등한지 유무 파악 함수
 * prependChild()            : 부모노드의 첫번재 자식노드로 삽입
 * insertAfter()             : 목표노드 뒤에 삽입노드 추가 
 * queryAll(), query(), $q() : 문서객체를 손쉽게 선택할 수 있도록 도와주는 함수
 * removeNode()              : 문서 객체(노드)를 제거하는 함수
 * createNode()              : 요소노드를 생성한 다음 내부에 텍스트노드를 자식 노드로 삽입
 * getStyle()                : 웹 브라우저에서 계산된 CSS 스타일 값 가져오는 함수
 * camelCase()               : CSS property name을 camelCase 바꿔주는 함수
 * errorMsg()                : error 메시지 함수
 * isElNode(), isntElNode()  : 요소노드인지 아닌지를 체크하여 참/거짓을 반환하는 함수
 * isElName()                : 요소노드의 이름이 동일한지 체크
 * isntElName()              : 요소노드의 이름이 아닌지 체크
 * isTextNode()              : 텍스트노드의 유형인지 체크
 * isntTextNode()            : 텍스트노드의 유형이 아닌지 체크
 * prevEl()                  : 이전 형제요소 노드를 찾는 함수(요소노드인 경우 반환)
 * nextEl()                  : 다음 형제요소 노드를 찾는 함수 (요소노드)
 * firstEl()                 : 첫번째 자식요소 노드를 찾는 헬퍼 함수
 * lastEl()                  : 마지막 자식요소 노드를 찾는 함수
 * getUnit()                 : 단위를 빼오는 함수
 * removeUnit()              : 단위 제거 함수 
 * hasUnit()                 : 단위값이 가지고 있는 유무 체크 함수
 * makeArray()               : 배열을 만들어주는 메소드
 * ----------------------------------------------------------------------------
 */

/**
 * -----------------------------------------------------------
 * Javascript의 모든 데이터 유형을 올바르게 감지 함수 
 * -----------------------------------------------------------
 */
...
/**
 * ---------------------------------------------
 * 웹 브라우저에서 계산된 CSS 스타일 값 가져오는 함수
 * ---------------------------------------------
 * 비 표준 MS IE 방식 (IE 8-)
...
/**
 * -------------------------------------------
 * 요소노드인지 아닌지를 체크하여 참/거짓을 반환하는 함수 - 0816
 * -------------------------------------------
 */
...
function isElName(node, name) {
  if ( isntElNode(node) ) { 
    errorMsg('첫번째 인자로 요소노드가 전달되어야 합니다.') 
  }
